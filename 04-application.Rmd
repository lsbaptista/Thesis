...
gle<-tail(GLE.PA)
gle <- knitr::kable(
  gle, caption = "Cotação Société Générale na Euronext-25-05-2020/01-06-2020",
  booktabs = TRUE)
...
returns <- returns[-1,]
colnames(returns)<-c("Telefónica","Enel","Société Générale ","Airbus")
```
\normalsize

```{r,volatilidade,echo=FALSE, fig.cap='Volatilidade nos últimos 4 anos',fig.topcaption = TRUE,out.width = "45%"}
par(mfrow  = c(2,2))
autoplot(returns$Telefónica,ts.colour = 'darkgreen') + ggtitle("Telefónica") + 
  ylab("Log return") + xlab("anos") + theme(plot.title = element_text(hjust = 0.5))
autoplot(returns$Enel,ts.colour = 'dodgerblue3') + ggtitle("Société Générale") + 
  ylab("Log return") + xlab("anos") + theme(plot.title = element_text(hjust = 0.5))
autoplot(returns$`Société Générale`,ts.colour = 'darkorange') + ggtitle("Enel") + 
  ylab("Log return") + xlab("anos") + theme(plot.title = element_text(hjust = 0.5))
autoplot(returns$Airbus,ts.colour = 'deeppink') + ggtitle("Airbus") + 
  ylab("Log return") + xlab("anos") + theme(plot.title = element_text(hjust = 0.5))
```
\FloatBarrier
\centering 
...
```{r,distr,echo=FALSE, fig.cap='Distribuição dos retornos logarítmicos',fig.topcaption = TRUE,out.width = "100%"}
par(mfrow  = c(2,2))
hist(returns$Telefónica,probability=T, main="Telefónica",xlab="",breaks=100)
lines(density(returns$Telefónica),col=2)
curve(dnorm(x,0,0.01695272), from = -0.10,to=0.10, col='blue',add = TRUE)
hist(returns$Enel,probability=T, main="Enel",xlab="",breaks=100)
lines(density(returns$Enel),col=2)
curve(dnorm(x,0,0.01571777), from = -0.05,to=0.05, col='blue',add = TRUE)
hist(returns$`Société Générale`,probability=T, main="Société Générale ",xlab="",breaks=100)
lines(density(returns$`Société Générale`),col=2)
curve(dnorm(x,0,0.02557792), from = -0.1,to=0.1, col='blue',add = TRUE)
hist(returns$Airbus,probability=T, main="Airbus",xlab="",breaks=100)
lines(density(returns$Airbus),col=2)
...
Por fim, o teste Shapiro-Wilk (teste à normalidade) na tabela \@ref(tab:shapiro) apresentam um valor de p<0.05 (considerando um nível significância a 5%), sendo que se pode rejeitar a distribuição normal dos índices.
...
A média móvel exponencial ponderada (EWMA) é calculado com um factor de decaimento $\lambda$ de 0.94 e de acordo com a equação \@ref(eq:ewma2). Para esse fim utiliza-se a função do R *ewmaVol* sendo calculado todos os valores de desvio padrão deste o inicio da serie temporal. 
...
```{r,gewma,echo=FALSE, fig.cap='Média móvel exponencial ponderada (EWMA)',fig.topcaption = TRUE,out.width = "100%"}
TEFew <- ewmaVol(returns$Telefónica,lambda = 0.94)
ENELew <- ewmaVol(returns$Enel,lambda = 0.94)
GLEew <- ewmaVol(returns$`Société Générale`,lambda = 0.94)
AIRew <- ewmaVol(returns$Airbus,lambda = 0.94)

par(mfrow  = c(2,2))
TEL1<-plot(returns$Telefónica,type="l",main="TEF.MC", col="green")
lines(TEFew$sigma,col="darkgreen")
TEL3<-plot(returns$Enel,type="l",main="ENEL.MI",col="green")
lines(ENELew$sigma,col="dodgerblue3")
TEL5<-plot(returns$`Société Générale`,type="l",main="GLE.PA",col="green")
lines(GLEew$sigma,col="darkorange")
TEL7<-plot(returns$Airbus,type="l",main="AIR.PA",col="green")
...
pewma<-t(pewma)
pewmak<- knitr::kable(
  pewma, caption = "Média móvel exponencial ponderada (EWMA)",
  booktabs = TRUE)
...
### Simulação Média móvel exponencial ponderada (EWMA)
...
TEF.MC.sigma <- dfewma$Telefónica*sqrt(252)
...
ENEL.MI.sigma <- dfewma$Enel*sqrt(252)
...
GLE.PA.sigma <- dfewma$`Société Générale `*sqrt(252)
...
AIR.PA.sigma <- dfewma$Airbus*sqrt(252)
...
SimEWMAk<- knitr::kable(
  tail(SimEWMA), caption = "Valores simulados com a variância obtida pelo método EWMA",
  booktabs = TRUE)
...
### Portefólio obtido pelo método EWMA
...
Na definição do portefólio vamos utilizar os dados referentes às simulações realizadas, considerando que o objectivo é minimizar o risco, ou seja minimizar a variância. A função utilizada pelo R é *minvariancePortfolio*, sendo desta forma definido os pesos de cada um dos activos que irão constituir o portefólio, apresentados na tabela \@ref(tab:pewma).
...
minewmak<- knitr::kable(
  minewma, caption = "Método EWMA - Pesos no portefólio",
  booktabs = TRUE)
...
Analisando o valor calculado para o portefólio, pode-se verificar que para um nível de confiança de 95%, o VaR a 1 dia para dados simulados a 80 dias é de `r MVaRewma_VaR`, ou seja, existe uma probabilidade de 5% de ocorrer uma perda igual ou superior a este valor no montante total do portefólio investido.

## Modelo de Heterocedasticidade condicional auto-regressiva (GARCH)
...
A aplicação do modelo é realizado com um modelo GARCH de primeira ordem, aplicando-se uma distribuição condicional dos retornos standardizados do tipo *t de Student*, sendo que o teste Jarque-Bera e o Shapiro-Wilk rejeitam para um nível de significância de 5% a normalidade dos dados, pois o valor *p* é inferior ao nível de significância, rejeitando-se deste modo $H_0$, ou seja, que os dados apresentam uma distribuição normal (ver Apêndice). A escolha deste tipo de distribuição deve-se ao facto dos retornos apresentarem uma distribuição do tipo leptocúrtica.(@rvol)
...
```{r,warning = FALSE}
#R CODE
TEF<-garchFit(formula = ~ garch(1,1),data=returns$Telefónica,cond.dist = "std",  include.mean=FALSE, 
              trace=FALSE)
```
\normalsize
...
TEFcoefk<- knitr::kable(
  TEFcoef, caption = "Telefónica - Coeficientes do modelo GARCH",
  booktabs = TRUE)
...
```{r, echo=FALSE}
pers <- TEF@fit$params$params[3]+TEF@fit$params$params[5]
```
...
TEFLjunk<- knitr::kable(
  TEFLjun, caption = "Telefónica - Teste de resíduos padronizados",
  booktabs = TRUE)
...
```{r,TELplot,echo=FALSE, fig.cap='Telefónica - Análise gráfica do modelo GARCH',fig.topcaption = TRUE,out.width = "100%"}
par(mfrow  = c(2,2))
plot(TEF,which= 2)
plot(TEF,which= 3)
plot(TEF,which=4)
...
```
O valor do desvio padrão diário que vamos utilizar para efeitos de simulação, é o valor de `r TEFvol`, sendo este o valor representativo da volatilidade condicional no último dia de cotação.
...
```{r,  warning = FALSE}
#R CODE
ENEL<-garchFit(formula = ~ garch(1,1),data=returns$Enel,cond.dist = "std", include.mean=FALSE, 
               trace=FALSE)
```
\normalsize
...
ENELcoefk<- knitr::kable(
  ENELcoef, caption = "Enel - Coeficientes do modelo GARCH",
  booktabs = TRUE)
...
ENELpers <-ENEL@fit$params$params[3]+ENEL@fit$params$params[5]
```
...
ENELLjunk<- knitr::kable(
  ENELLjun, caption = "Enel - Teste de resíduos padronizados",
  booktabs = TRUE)
...
```{r,ENELplot,echo=FALSE, fig.cap='Enel- Análise gráfica do modelo GARCH',fig.topcaption = TRUE,out.width = "100%"}
par(mfrow  = c(2,2))
plot(ENEL,which= 2)
plot(ENEL,which= 3)
plot(ENEL,which=4)
...
Ao realizar o ajuste do modelo GARCH(1,1) com o modelo AR(1) a estimação do modelo obteve uma boa resposta na modelação da volatilidade, sendo que todos os parâmetros do modelo, nomeadamente o $\alpha$ e o $\beta$ apresentam valores *p* bastante baixos, sendo significantes a um nível inferior a 5%, evidenciando que o modelo com os parâmetros apresentados providenciam um bom ajuste aos dados, como podemos ver na tabela \@ref(tab:GLEcoefk).

\scriptsize
```{r,warning = FALSE}
#R CODE
GLE<-garchFit(formula = ~ arma(1,0) + garch(1,1),data=returns$`Société Générale`,
...
GLEcoefk<- knitr::kable(
  GLEcoef, caption = "Société Générale - Coeficientes do modelo GARCH",
  booktabs = TRUE)
...
```{r,echo=FALSE}
GLEpers <- GLE@fit$params$params[3]+GLE@fit$params$params[5]
```
...
GLELjunk<- knitr::kable(
  GLELjun, caption = "Société Générale - Teste de resíduos padronizados ",
  booktabs = TRUE)
...
```{r,GLEplot,echo=FALSE, fig.cap='Société Générale - Análise gráfica do modelo GARCH',fig.topcaption = TRUE,out.width = "100%"}
par(mfrow  = c(2,2))
plot(GLE,which= 2)
plot(GLE,which= 3)
plot(GLE,which=4)
...
AIR <- ugarchfit(AIR.spec, returns$Airbus) 
```
\normalsize
...
AIRcoefk<- knitr::kable(
  AIRcoef, caption = "Airbus - Coeficientes do modelo GARCH",
  booktabs = TRUE)
...
```{r,echo=FALSE}
AIRpers <- AIR@fit$params$params[3]+AIR@fit$params$params[5]
```
...
Na tabela \@ref(tab:AIRLjunk) podemos verificar que as estatísticas para o teste Ljung-Box demonstram que o modelo integrou a volatilidade dos retornos. No apêndice I, na tabela referente ao *output* do modelo o teste referente ao LM-ARCH também podemos confirmar que os resíduos estandardizados se comportam como um ruido branco, não se podendo rejeitar a hipótese nula visto não haver evidência de auto-correlação.
...
AIRLjunk<- knitr::kable(
  AIRLjun, caption = "Airbus - Teste de resíduos padronizados",
  booktabs = TRUE)
...
Os gráficos ACF vêm complementar a informação relativa aos teste Ljung-Box.
```{r,AIRplot,echo=FALSE, fig.cap='Airbus - Análise gráfica do modelo GARCH',fig.topcaption = TRUE,out.width = "100%"}
par(mfrow  = c(2,2))
plot(AIR,which= 1)
plot(AIR,which= 3)
plot(AIR,which=10)
...
SimGARCHk<- knitr::kable(
  tail(SimGARCH), caption = "Valores simulados com a variância obtida pelo método GARCH",
  booktabs = TRUE)
...
```{r,frontgarch,echo=FALSE, fig.cap='GARCH - Fronteira eficiente',fig.topcaption = TRUE,out.width = "70%"}
...
minGARCHk<- knitr::kable(
  minGARCH, caption = "GARCH - Pesos no portefólio",
  booktabs = TRUE)
...
Para um nível de confiança 95%, o VaR a 1 dia para dados simulados a 80 dias é de `r MVaRGARCH_VaR`, ou seja, existe uma probabilidade de 5% de ocorrer uma perda igual ou superior a este valor no montante total do portefólio investido.

## Volatilidade implicita

A volatilidade implícita é calculada de acordo com o modelo de Black-Scholes para opções europeias, sendo que o cálculo foi efectuado utilizando os valores presentes na tabela \@ref{fig:opcao}. Os valores apresentados referem-se ao valor de opções de compra^[https://www.eurexchange.com/exchange-en/products/equ/opt], para preço de exercício próximos ou iguais aos valores das acções, ou seja, *at-the-money*. A data de vencimento das opções, ou maturidade, é a 18/09/2020.
...
SimVIk<- knitr::kable(
  tail(SimVI), caption = "Valores simulados com a variância obtida pelo método da volatilidade implicita",
  booktabs = TRUE)
...
### Portefólio volatilidade ímplicita
...
minVIk<- knitr::kable(
  minVI, caption = "Volatilidade Implicita- Pesos no portefólio",
  booktabs = TRUE)
